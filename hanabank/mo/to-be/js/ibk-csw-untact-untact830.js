/*****************************************************************************
 * 파일명 : ibk-csw-untact-untact830.js
 * 작성일 : 2021. 04. 23
 * 작성자 : 신성환
 * 설 명 : 서류재촬영 - 등기사항전부증명서
 * ===========================================================================
...
ibk.csw.untact.untact830 = function(){
	
	return{
		
		inputChange : function (obj){
			var tempNum = obj.value.trim();
			tempNum = tempNum.replace(/[^0-9a-zA-Z]/g,'').toUpperCase();
			obj.value = tempNum; 
			if(obj.value.length > 4 && obj.value.length < 9){
				var team =  tempNum.substring(0,4) + "-" + tempNum.substring(4,8);
				obj.value = team;
			}else if(obj.value.length > 8 && obj.value.length < 20){
				var team =  tempNum.substring(0,4) + "-" + tempNum.substring(4,8) + "-" + tempNum.substring(8,12);
				obj.value = team;
			}			
		},		
							...
							if(fileName.substring(fileName.indexOf(".")+1) == "pdf"){
					 			var imgTag = '<li class="swiper-slide" id="swiper-slide_'+imgCnt+'" name="swiper-slide">' + 
								 			 	...
								 			 	'<img src="'+e.target.result+'" name="apnxDcmtImg" />'+
								 					'<button type="button" class="img_del" value="'+fileName+'" onclick="ibk.csw.untact.untact830.apnxDcmtDel(this);">삭제</button>'+
								 				'</div>'+
								 			 '</li>';					
							}else{
					 			var imgTag = '<li class="swiper-slide" id="swiper-slide_'+imgCnt+'" name="swiper-slide">' + 
		...
		enprDcmtSbmt : function (formObj){
			...
			if($("#cersIssuNo").val() == ""){
				hanaDialog.openAlert({title:"알림", message:"법인등기사항전부증명서 발급번호는 필수 입력 입니다."});
				return false;
			}			
			
			if($("#cersIssuNo").val().length < 14){
				hanaDialog.openAlert({title:"알림", message:"법인등기사항전부증명서 발급번호 12자리를 입력하여 주세요."});
				return false;
			}			
			...
			hanaJQuery.ajaxSubmit("/untact/csuntact515.do", formObj, function(httpRequest, textStatus) {
				...
				try{
					...
					// 서류제출 실행						
	 				ibk.csw.untact.untact830.enprDcmtSbmtExec(formObj,webCrypto,1,imgCnt);
						...
						try{
							...
							if (data.result == 'success') {
								if(imgIdx == imgCnt){
									...
									msb.util.form.createHiddenField(formObj, 'evdcDcmtSbmtYn', '3');
									...
									// 다음건  서류제출 실행
									HanaAppInterface.endLoadingDialog();
	 								ibk.csw.untact.untact830.enprDcmtSbmtExec(formObj,webCrypto,(imgIdx+1),imgCnt);
								}
							}else if(data.result == 'error'){
								HanaAppInterface.endLoadingDialog();
...
//# sourceURL=ibk-csw-untact-untact830.js