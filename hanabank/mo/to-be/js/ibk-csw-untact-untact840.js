/*****************************************************************************
 * 파일명 : ibk-csw-untact-untact840.js
 * 작성일 : 2021. 04. 23
 * 작성자 : 신성환
 * 설 명 : 서류재촬영 - 실제소유자확인 서류
 * ===========================================================================
 * 변경이력:
...
ibk.csw.untact.untact840 = function(){
	...
	return{
							...
							if(fileName.substring(fileName.indexOf(".")+1) == "pdf"){
					 			var imgTag = '<li class="swiper-slide" id="swiper-slide_'+imgCnt+'" name="swiper-slide">' + 
								 			 	...
								 			 	'<img src="'+e.target.result+'" name="apnxDcmtImg" />'+
								 					'<button type="button" class="img_del" value="'+fileName+'" onclick="ibk.csw.untact.untact840.apnxDcmtDel(this);">삭제</button>'+
								 				'</div>'+
								 			 '</li>';					
							}else{
					 			var imgTag = '<li class="swiper-slide" id="swiper-slide_'+imgCnt+'" name="swiper-slide">' + 
		...
		enprDcmtSbmt : function (formObj){
			...
			if($("#extgNewCustDvCd").val() == "2" && $("#shareRtCorcPossYn").val() == "Y"){
				if($("#shareRt").val() == "" || $("#shareRt").val() == "0"){
					hanaDialog.openAlert({title:"알림", message:"지분율은 필수 입력 입니다."});
					return false;
				}
			}
			...
			hanaJQuery.ajaxSubmit("/untact/csuntact515.do", formObj, function(httpRequest, textStatus) {
				...
				try{
					...
					// 서류제출 실행						
	 				ibk.csw.untact.untact840.enprDcmtSbmtExec(formObj,webCrypto,1,imgCnt);
						...
						try{
							...
							if (data.result == 'success') {
								if(imgIdx == imgCnt){
									...
									msb.util.form.createHiddenField(formObj, 'evdcDcmtSbmtYn', '4');
									...
									hanaJQuery.ajaxSubmit("/untact/csuntact516.do", formObj, function(httpRequest, textStatus) {
										...
										if (data.result == 'success') {
											...
											if($("#extgNewCustDvCd").val() == "2"){
												...
												var hanaJQuery = new HanaJQuery(msb.HANA_CONTENT, true, null);
												hanaJQuery.ajaxSubmit("/untact/csuntact841.do", formObj, function(httpRequest, textStatus) {
									...
									// 다음건  서류제출 실행
									HanaAppInterface.endLoadingDialog();
	 								ibk.csw.untact.untact840.enprDcmtSbmtExec(formObj,webCrypto,(imgIdx+1),imgCnt);
								}
							}else if(data.result == 'error'){
								HanaAppInterface.endLoadingDialog();
...
//# sourceURL=ibk-csw-untact-untact840.js