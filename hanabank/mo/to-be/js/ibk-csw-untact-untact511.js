/*****************************************************************************
 * 파일명 : ibk-csw-untact-untact511.js
 * 작성일 : 2021. 04. 08
 * 작성자 : 신성환
 * 설 명 : 인증(사업자서류제출) - 등기사항전부증명서
 * ===========================================================================
 * 변경이력:
...
ibk.csw.untact.untact511 = function(){
							...
							if(fileName.substring(fileName.indexOf(".")+1) == "pdf"){
					 			var imgTag = '<li class="swiper-slide" id="swiper-slide_'+imgCnt+'" name="swiper-slide">' + 
								 			 	...
								 			 	'<img src="'+e.target.result+'" name="apnxDcmtImg" />'+
								 					'<button type="button" class="img_del" value="'+fileName+'" onclick="ibk.csw.untact.untact511.apnxDcmtDel(this);">삭제</button>'+
								 				'</div>'+
								 			 '</li>';					
							}else{
					 			var imgTag = '<li class="swiper-slide" id="swiper-slide_'+imgCnt+'" name="swiper-slide">' + 
		...
		enprDcmtSbmt : function (formObj){
			...
			hanaJQuery.ajaxSubmit("/untact/csuntact515.do", formObj, function(httpRequest, textStatus) {
				...
				try{
					...
					// 서류제출 실행						
	 				ibk.csw.untact.untact511.enprDcmtSbmtExec(formObj,webCrypto,1,imgCnt);
								...
								if(imgIdx == imgCnt){
									...
									hanaJQuery.ajaxSubmit("/untact/csuntact516.do", formObj, function(httpRequest, textStatus) {
										...
										if (data.result == 'success') {
											// 다음 서류 제출 화면 이동
											HanaAppInterface.startLoadingDialog();
											msb.util.form.sendForm(formObj, '/untact/csuntact512.do');
									...
									// 다음건  서류제출 실행
									HanaAppInterface.endLoadingDialog();
	 								ibk.csw.untact.untact511.enprDcmtSbmtExec(formObj,webCrypto,(imgIdx+1),imgCnt);
								}
							}else if(data.result == 'error'){
								HanaAppInterface.endLoadingDialog();
...
//# sourceURL=ibk-csw-untact-untact511.js