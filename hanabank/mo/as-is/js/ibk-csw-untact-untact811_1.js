/*****************************************************************************
 * 파일명 : ibk-csw-untact-untact811.js
 * 작성일 : 2021. 04. 08
 * 작성자 : 신성환
 * 설 명 : 신분증 재촬영 - 주민등록증,운전면허증 촬영 안내 / 확인
 * ===========================================================================
 * 변경이력:
...
ibk.csw.untact.untact811 = function(){
...
//					$('#cameraImg').attr('src', dataUrl);
			...
			encDataPromise.then(function(encData) {
				...
				hanaJQuery.ajaxSubmit("/untact/csuntact812.do", document.getElementById("untact811Form"), function(httpRequest, textStatus){
					...
					try{
						...
						if(oPLATFORM.IPHONE()){
							location.href = "native://emptyTmpDirectory?cbName=ibk.csw.untact.untact811.nativeCallback";
						}						
...
//								$('#cameraImg').attr('src', data.maskImg);
			        		...
			        		hanaDialog.openAlert({title:"알림", message:"대표자의 실명번호가 상이합니다.", fCallback1:function(){	
			        			msb.util.form.sendForm(document.getElementById("untact811Form"), '/untact/untact_index.do?nftfChnlKindCd=MW02809A01');
			        		}});
			        		return false;							
		...
		idcdCameraWebRtc : function(imgData) {
			...
			SCCipherInit(webUrl, webCrypto).then(function(){
				...
				var abData = SCCipher.Encoder.AB.fromB64(imgData);
		...
		idcdTnfCnfm : function(formObj){
			...
			hanaJQuery.ajaxSubmit("/untact/csuntact515.do", formObj, function(httpRequest, textStatus) {
				...
				try{
					...
					var hanaJQuery = new HanaJQuery(msb.HANA_CONTENT, true, null);
					hanaJQuery.ajaxSubmit("/untact/csuntact813.do", formObj, function(httpRequest, textStatus) {
				        ...
				        try {
				        	...
				        	if(oJsonData.errorCode == 'BNFT20053'){
				        		...
				        		hanaDialog.openAlert({title:"알림", message:errMsg, fCallback1:function(){
				        			ibk.csw.untact.untact811.idcdCameraStart();
				        		}});
				        		return false;
				        	}
...
//# sourceURL=ibk-csw-untact-untact811.js