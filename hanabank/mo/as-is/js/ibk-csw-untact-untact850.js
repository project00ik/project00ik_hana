/*****************************************************************************
 * 파일명 : ibk-csw-untact-untact850.js
 * 작성일 : 2021. 04. 23
 * 작성자 : 신성환
 * 설 명 : 서류재촬영 - 금융거래목적증빙 서류
 * ===========================================================================
 * 변경이력:
...
ibk.csw.untact.untact850 = function(){
	...
	return{
		...
		inputChange : function (obj){
			...
			if(obj.value.length > 4 && obj.value.length < 8){
				var team =  tempNum.substring(0,4) + "-" + tempNum.substring(4,7);
				obj.value = team;
			}else if(obj.value.length > 7 && obj.value.length < 12){
				var team =  tempNum.substring(0,4) + "-" + tempNum.substring(4,7) + "-" + tempNum.substring(7,11);
				obj.value = team;
			}else if(obj.value.length > 11 && obj.value.length < 20){
				var team =  tempNum.substring(0,4) + "-" + tempNum.substring(4,7) + "-" + tempNum.substring(7,11) + "-" + tempNum.substring(11);
				obj.value = team;
			}			
		},		
			...
			for(var idx = 0; idx < filesLength; idx++){
				...
				promise.then(function (result){
					...
					if($("#swiperUl").find("img[name=apnxDcmtImg]").length > 0){
						$('#inputRadio-G2-01').addClass('disabled').prop('disabled','disabled');
						$('#inputRadio-G2-02').addClass('disabled').prop('disabled','disabled');
					}else{
						$('#inputRadio-G2-01').addClass('disabled').prop('disabled','');
						$('#inputRadio-G2-02').addClass('disabled').prop('disabled','');
					}					
				})
							...
							if(fileName.substring(fileName.indexOf(".")+1) == "pdf"){
					 			var imgTag = '<li class="swiper-slide" id="swiper-slide_'+imgCnt+'" name="swiper-slide">' + 
								 			 	...
								 			 	'<img src="'+e.target.result+'" name="apnxDcmtImg" />'+
								 					'<button type="button" class="img_del" value="'+fileName+'" onclick="ibk.csw.untact.untact850.apnxDcmtDel(this);">삭제</button>'+
								 				'</div>'+
								 			 '</li>';					
							}else{
					 			var imgTag = '<li class="swiper-slide" id="swiper-slide_'+imgCnt+'" name="swiper-slide">' + 
		...
		enprDcmtSbmt : function (formObj){
			...
			if($("#cersIssuNo").val() == ""){
				hanaDialog.openAlert({title:"알림", message:"금융거래목적증빙 서류 발급번호는 필수 입력 입니다."});
				return false;
			}			
			
			if($("#cersIssuNo").val().length < 17){
				hanaDialog.openAlert({title:"알림", message:"금융거래목적증빙 서류 발급번호 14자리를 입력하여 주세요."});
				return false;
			}			
			
			if($('#inputRadio-G2-01').is(':checked')){
				$("#crpnNftfEvdcDcmtKindCd").val("52");
			}else if($('#inputRadio-G2-02').is(':checked')){
				$("#crpnNftfEvdcDcmtKindCd").val("51");
			}		
			...
			hanaJQuery.ajaxSubmit("/untact/csuntact515.do", formObj, function(httpRequest, textStatus) {
				...
				try{
					...
					// 서류제출 실행						
	 				ibk.csw.untact.untact850.enprDcmtSbmtExec(formObj,webCrypto,1,imgCnt);
						...
						try{
							...
							if (data.result == 'success') {
								if(imgIdx == imgCnt){
									...
									msb.util.form.createHiddenField(formObj, 'evdcDcmtSbmtYn', '5');
									...
									hanaJQuery.ajaxSubmit("/untact/csuntact516.do", formObj, function(httpRequest, textStatus) {
										...
										if (data.result == 'success') {
											...
											// 모든 서류제출 완료
											msb.util.form.createHiddenField(formObj, 'nftfDcmtSbmtCpltYn', "Y");	
											...
											hanaJQuery.ajaxSubmit("/untact/csuntact516.do", formObj, function(httpRequest, textStatus) {
												...
												if (data.result == 'success') {
													// 서류재촬영 안내 화면 이동
													HanaAppInterface.startLoadingDialog();
													msb.util.form.sendForm(formObj, '/untact/csuntact800.do');
									...
									// 다음건  서류제출 실행
									HanaAppInterface.endLoadingDialog();
	 								ibk.csw.untact.untact850.enprDcmtSbmtExec(formObj,webCrypto,(imgIdx+1),imgCnt);
								}
							}else if(data.result == 'error'){
								HanaAppInterface.endLoadingDialog();
...
//# sourceURL=ibk-csw-untact-untact850.js