/*****************************************************************************
 * 파일명 : ibk-csw-untact-untact513.js
 * 작성일 : 2021. 04. 08
 * 작성자 : 신성환
 * 설 명 : 인증(사업자서류제출) - 금융거래목적증빙 서류
 * ===========================================================================
 * 변경이력:
...
ibk.csw.untact.untact513 = function(){
							...
							if(fileName.substring(fileName.indexOf(".")+1) == "pdf"){
					 			var imgTag = '<li class="swiper-slide" id="swiper-slide_'+imgCnt+'" name="swiper-slide">' + 
								 			 	...
								 			 	'<img src="'+e.target.result+'" name="apnxDcmtImg" />'+
								 					'<button type="button" class="img_del" value="'+fileName+'" onclick="ibk.csw.untact.untact513.apnxDcmtDel(this);">삭제</button>'+
								 				'</div>'+
								 			 '</li>';					
							}else{
					 			var imgTag = '<li class="swiper-slide" id="swiper-slide_'+imgCnt+'" name="swiper-slide">' + 
		...
		enprDcmtSbmt : function (formObj){
			...
			hanaJQuery.ajaxSubmit("/untact/csuntact515.do", formObj, function(httpRequest, textStatus) {
				...
				try{
					...
					// 서류제출 실행						
	 				ibk.csw.untact.untact513.enprDcmtSbmtExec(formObj,webCrypto,1,imgCnt);
						...
						try{
							...
							if (data.result == 'success') {
								if(imgIdx == imgCnt){
											...
											hanaJQuery.ajaxSubmit("/untact/csuntact516.do", formObj, function(httpRequest, textStatus) {
												...
												if (data.result == 'success') {
													...
													if($('#extgNewCustDvCd').val() == "1"){
														msb.util.form.sendForm(formObj, '/untact/csuntact521.do');
													// 신규고객 추가인증 안내 화면 이동
													}else{
									...
									// 다음건  서류제출 실행
									HanaAppInterface.endLoadingDialog();
	 								ibk.csw.untact.untact513.enprDcmtSbmtExec(formObj,webCrypto,(imgIdx+1),imgCnt);
								}
							}else if(data.result == 'error'){
								HanaAppInterface.endLoadingDialog();
...
//# sourceURL=ibk-csw-untact-untact513.js