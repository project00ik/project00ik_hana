/*****************************************************************************
 * 파일명 : ibk-csw-untact-untact820.js
 * 작성일 : 2021. 04. 23
 * 작성자 : 신성환
 * 설 명 : 서류재촬영 - 사업자등록증명원
 * ===========================================================================
...
ibk.csw.untact.untact820 = function(){
	...
	return{
							...
							if(fileName.substring(fileName.indexOf(".")+1) == "pdf"){
					 			var imgTag = '<li class="swiper-slide" id="swiper-slide_'+imgCnt+'" name="swiper-slide">' + 
								 			 	...
								 			 	'<img src="'+e.target.result+'" name="apnxDcmtImg" />'+
								 					'<button type="button" class="img_del" value="'+fileName+'" onclick="ibk.csw.untact.untact820.apnxDcmtDel(this);">삭제</button>'+
								 				'</div>'+
								 			 '</li>';					
							}else{
					 			var imgTag = '<li class="swiper-slide" id="swiper-slide_'+imgCnt+'" name="swiper-slide">' + 
		...
		enprDcmtSbmt : function (formObj){
			...
			if($("#cersIssuNo").val() == ""){
				hanaDialog.openAlert({title:"알림", message:"사업자등록증명원 발급번호는 필수 입력 입니다."});
				return false;
			}			
			
			if($("#cersIssuNo").val().length < 17){
				hanaDialog.openAlert({title:"알림", message:"사업자등록증명원 발급번호 14자리를 입력하여 주세요."});
				return false;
			}			
			...
			hanaJQuery.ajaxSubmit("/untact/csuntact515.do", formObj, function(httpRequest, textStatus) {
				...
				try{
					...
					// 서류제출 실행						
	 				ibk.csw.untact.untact820.enprDcmtSbmtExec(formObj,webCrypto,1,imgCnt);
						...
						try{
							...
							if (data.result == 'success') {
								if(imgIdx == imgCnt){
									...
									msb.util.form.createHiddenField(formObj, 'evdcDcmtSbmtYn', '2');
									...
									// 다음건  서류제출 실행
									HanaAppInterface.endLoadingDialog();
	 								ibk.csw.untact.untact820.enprDcmtSbmtExec(formObj,webCrypto,(imgIdx+1),imgCnt);
								}
							}else if(data.result == 'error'){
								HanaAppInterface.endLoadingDialog();
...
//# sourceURL=ibk-csw-untact-untact820.js